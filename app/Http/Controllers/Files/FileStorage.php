...
class FileStorage extends Controller
{
        ...
        if (env("DIR_PATH_FILE") == "local") {
            ...
            try {
                ...
                if (count($image_parts) < 2) {
                    // Try to handle if it doesn't have the preamble, or throw
                    if (base64_encode(base64_decode($fileb64, true)) === $fileb64) {
    ...
    public static function delete($path_url, $file_id = null)
    {
        if (env("DIR_PATH_FILE") == "imagekit") {
            if (!$file_id && strpos($path_url, "ik.imagekit.io") !== false) {
                // Try to extract file_id if not provided, or logic might fail if we don't store it
                // For now assume file_id is passed
                return "falla: file_id requerido para imagekit";
            }
            ...
            // Local storage
            $host     = url("/");
            // Remove host from path to get relative public path
            $pathFile = str_replace($host . '/', '', $path_url);
